/**
 * Both KB Securities and VCI return timestamps with no timezone designator
 * (e.g. "2025-12-31T00:00:00", "2026-03-10T16:56:34.897") across every
 * endpoint that returns them (finance report periods, company
 * profile/shareholder/ownership records). `new Date(...)` would parse that
 * as the executing machine's local time, not Vietnam's — pin it to +07:00
 * explicitly instead (Vietnam has no DST).
 */
